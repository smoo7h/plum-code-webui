...
**Admin / helper LLM** (`packages/backend/src/utils/adminLLM.ts`): one-shot text completion for internal features (commit message generation, etc.). Prefers Claude → Codex → OpenCode → Vibe; override via `ADMIN_LLM_PROVIDER`. Used by `routes/git.ts` `/generate-commit-message` — no longer hardcoded to `claude --print -p`.
